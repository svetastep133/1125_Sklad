...
public class IncomingRepository
{
    ...
    public List<Incoming> GetIncoming()
    {
        ...
        string sql = "SELECT i.Id, i.Doc_number, i.Date, i.Status, i.Supplier_id, i.Warehouse_id, w.Name,s.Name as sName, s.Id as sId from Incoming i inner join Suppliers s on i.Supplier_id = s.Id inner join Warehouses w on i.Warehouse_id = w.Id ";
        try
        {
            connection.Open();
